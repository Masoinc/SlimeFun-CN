...
public abstract class AReactor extends SlimefunItem {
			...
			public void newInstance(final BlockMenu menu, final Block b) {
				try {
					...
					if (!BlockStorage.hasBlockInfo(b) || BlockStorage.getBlockInfo(b, "reactor-mode").equals("generator")) {
						...
						menu.replaceExistingItem(4, new CustomItem(SlimefunItems.PLUTONIUM, "&7模式: &3生产", "", "&8◇ &7你的核反应堆将会偏重于生产产物", "&8◇ &7即使你的线路不缺少电力", "&8◇ &7核反应仍将继续运行", "&8◇ &7但不会产生电能", "","&8-> &6点击&7将模式切换为 &2发电"));
						menu.addMenuClickHandler(4, new MenuClickHandler() {
	...
	private void constructMenu(BlockMenuPreset preset) {
		...
		preset.addItem(1, new CustomItem(SlimefunItems.URANIUM, "&8[ &2核燃料槽 &8]",  "&8◇ &7请在此处放入核燃料如:", "&8- &2铀","&8- &c镎"),
		new MenuClickHandler() {
		...
		if (needsCooling()) {
			preset.addItem(7, new CustomItem(this.getCoolant(), "&8[ &3冷却槽 &8]", "", "&8◇ &7请在此处放入:","&8- &b核反应堆冷却元件", "&8◇ &7反应堆在无冷却元件时运作","&8◇ 有可能发生 &c爆炸"));
		}
		else {
            preset.addItem(7, new CustomItem(new MaterialData(Material.BARRIER), "&8[ &3冷却槽 &8]", "", "&8◇ &7请在此处放入:","&8- &b核反应堆冷却元件"));

            for (int i : border_4) {
                preset.addItem(i, new CustomItem(new ItemStack(Material.BARRIER), "&8[ &6不需要冷却 &8]"),
                new MenuClickHandler() {
